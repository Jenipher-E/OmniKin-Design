/* OmniKin · Feature 3 — static all-screens gallery on the design canvas. */
const { F3_SCREENS, F3_GALLERY, OK_THEMES, OK_FONT, OK_MONO, DeviceFrame } = window;
  ...
  return (
    <div style={{ position: 'fixed', top: 16, left: 16, zIndex: 1000, display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,.86)', backdropFilter: 'blur(12px)', borderRadius: 14, padding: '10px 12px', boxShadow: '0 4px 20px rgba(14,58,64,.14), inset 0 0 0 1px rgba(14,58,64,.06)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, paddingLeft: 4, paddingRight: 6 }}>
        ...
        <div style={{ fontFamily: OK_FONT, fontSize: 13.5, fontWeight: 700, color: '#1C6E78', letterSpacing: -0.2 }}>Shopping Lists · All screens</div>
      </div>
      ...
      <div style={{ width: 1, height: 24, background: 'rgba(14,58,64,.12)' }} />
      <a href="OmniKin Shopping Lists Prototype.html" style={{ textDecoration: 'none', fontFamily: OK_FONT, fontSize: 13, fontWeight: 600, color: '#fff', background: '#E2683C', borderRadius: 9, padding: '8px 14px' }}>Open prototype →</a>
    </div>
  );
}
      ...
      <window.DesignCanvas>
        {F3_GALLERY.map((sec) => (
          <window.DCSection key={sec.id} id={sec.id} title={sec.title} subtitle={sec.subtitle}>
            {sec.screens.map((sid) => (
              <window.DCArtboard key={sid + platform + theme} id={sid} label={F3_SCREENS[sid].label} width={devW} height={devH} style={{ background: 'transparent', boxShadow: 'none', borderRadius: 0 }}>
                <DeviceFrame platform={platform} t={t} dark={theme === 'dark'} scale={1}>
                  <div style={{ position: 'absolute', inset: 0 }}>
                    {F3_SCREENS[sid].render({ t, platform, dark: theme === 'dark', go: noop, back: noop })}
                  </div>
                </DeviceFrame>
              </window.DCArtboard>
            ))}
