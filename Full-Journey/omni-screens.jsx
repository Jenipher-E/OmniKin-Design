  ...
  return (
    <Screen t={t} platform={platform}>
      ...
      <Pad style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Btn t={t} kind="primary" onClick={() => go('homeFresh')}>{isFace ? 'Enable Face ID' : 'Enable fingerprint'}</Btn>
        <Btn t={t} kind="ghost" onClick={() => go('homeFresh')} style={{ height: 48 }}>Not now</Btn>
      </Pad>
      <Spacer h={14} />
    </Screen>
  );
...
/* ════════════════════ HOME (Owner landing) ════════════════════ */
function HomeSpace({ t, platform, household = 'Reuter Family', owner = 'Anna', ownerColor = '#1C6E78', members, offline, fresh, go }) {
  members = members || [{ n: 'Anna', c: '#1C6E78', role: 'Owner' }];
  const tiles = [
    { label: 'Calendar', icon: I.cal, color: t.teal700, bg: t.panelTeal, empty: 'No events yet' },
    { label: 'Lists', icon: I.list, color: t.blue, bg: t.panelBlue, empty: 'No lists yet' },
    { label: 'Documents', icon: I.doc, color: t.coral, bg: t.panelCoral, lock: true, empty: 'Nothing saved' },
    { label: 'Settings', icon: I.gear, color: t.green, bg: t.panelGreen, lock: true, empty: 'Set up' },
  ];
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {fresh && (
          <React.Fragment>
            <Spacer h={16} />
            <Pad>
              <div style={{ background: t.surface, border: `1px solid ${t.hairline}`, borderRadius: 18, padding: '20px 18px', boxShadow: t.shadowSm, textAlign: 'center' }}>
                <div style={{ width: 60, height: 60, borderRadius: 18, background: t.panelTeal, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 13 }}>{I.home(t.teal700, 26)}</div>
                <div style={{ fontFamily: OK_FONT, fontSize: 19, fontWeight: 700, color: t.title, letterSpacing: -0.3 }}>Your household is ready</div>
                <div style={{ fontFamily: OK_FONT, fontSize: 13.5, color: t.slate, lineHeight: 1.5, marginTop: 7, maxWidth: 290, marginLeft: 'auto', marginRight: 'auto' }}>It's just you so far. Nothing has been added yet. Invite the people you share a home with, then start filling in your calendar, lists and documents.</div>
                <div style={{ marginTop: 16 }}><Btn t={t} kind="primary" icon={I.plus('#fff', 18)} onClick={() => go('home')}>Invite your family</Btn></div>
              </div>
            </Pad>
          </React.Fragment>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {tiles.map((tile) => (
              <div key={tile.label} style={{ background: t.surface, border: `1px solid ${t.hairline}`, borderRadius: 16, padding: 15, height: 92, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: t.shadowSm }}>
                ...
                <div>
                  <span style={{ fontFamily: OK_FONT, fontSize: 14.5, fontWeight: 600, color: t.ink, display: 'block' }}>{tile.label}</span>
                  {fresh && <span style={{ fontFamily: OK_FONT, fontSize: 11.5, color: t.muted, display: 'block', marginTop: 1 }}>{tile.empty}</span>}
                </div>
              </div>
            ))}
          </div>
...
const SCREENS = {
  ...
  homeFresh:     { render: (p) => <HomeSpace {...p} fresh />, label: 'Household ready · clear state' },
...
const BACK = {
  ...
  euSignup: 'welcomeA', euDisplayName: 'euSignup', euVerify: 'euDisplayName', biometric: 'euVerify', homeFresh: 'biometric',
...
const GALLERY = [
  ...
  { id: 'biohome', title: 'Biometrics & home space', subtitle: 'Shared across every flow', screens: ['biometric', 'homeFresh', 'home'] },
  ...
  { id: 'clear', title: 'First arrival · clear state', subtitle: 'Straight after onboarding: the household exists but nothing has been added yet. It is just the Owner, with one clear way to start', screens: ['homeFresh'] },
