  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        ...
        apply();
        // Guard against a persisted pan/zoom that strands every artboard
        // off-screen (e.g. zoomed to minScale then panned far away). The
        // artboards mount a beat later (DesignCanvas gates on a sidecar read),
        // so poll a few frames: once slots exist, if none intersect the
        // viewport, drop the saved transform and recenter on content.
        let tries = 0;
        const recenterIfStranded = () => {
          const vp = vpRef.current;
          if (!vp) return;
          const slots = document.querySelectorAll('[data-dc-slot],[data-dc-section]');
          if (!slots.length) { if (tries++ < 40) requestAnimationFrame(recenterIfStranded); return; }
          const r = vp.getBoundingClientRect();
          let anyVisible = false;
          slots.forEach((el) => {
            const b = el.getBoundingClientRect();
            if (b.right > r.left && b.left < r.right && b.bottom > r.top && b.top < r.bottom) anyVisible = true;
          });
          if (!anyVisible) {
        ...
        requestAnimationFrame(recenterIfStranded);
      }
    } catch {}
